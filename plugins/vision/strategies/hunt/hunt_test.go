...
// run installs world+vision, registers hunt, spawns a predator at hunter
...
func run(t *testing.T, hunter body, prey []body, bystanders []body) geom.Vec {
	...
	systems = append(systems, goke.SystemFn{OnInit: func(si *goke.SysInit) {
		query = si.NewQueryBuilder(&vel).Include(goke.Include[hunt.Predator]()).Build()
	}})
