...
// System turns State into Selected tags — the actual query+migrate happens
...
var _ goke.Module = (*System)(nil)
var _ goke.System = (*System)(nil)

// NewSystem builds a System driven by state — see Plugin for how state gets published and updated.
func NewSystem(state *State) *System { return &System{state: state} }

func (s *System) bindSpace(space *gokg.Space)     { s.space = space }
func (s *System) bindCamera(camera render.Camera) { s.camera = camera }
...
func (s *System) Select(ids []uid.UID64) {
...
func (s *System) applySelection(cb *goke.CmdBuf, hit map[uid.UID64]struct{}, additive bool) {
	s.query.All()
	for s.query.Next() {
...
func (s *System) worldBox(start, end geom.Vec[int32]) geom.AABB[uint32] {
...
func (s *System) RegSystems(ecs *goke.ECS) {
	if s.sys == nil {
		s.sys = ecs.RegSys(s)
	}
}
...
func (s *System) RunPlan(ctx goke.RunCtx, d time.Duration) {
	ctx.Run(s.sys, d)
	ctx.Sync()
}

// SetupSystems is empty — selection has no one-time seeding of its own.
func (s *System) SetupSystems() []goke.System { return nil }

// LoadComps lists the component types selection owns — see [goke.CompProvider].
func (s *System) LoadComps() []goke.CompToken {
