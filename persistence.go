...
func (p *Persistence) Save(basePath, label string, resources ...any) error {
	all := append(p.game.pluginManager.saveTargets(), p.game.resources.Persisted()...)
	all = append(all, resources...)
	return persist.Save(p.game.ecs, basePath, label, all...)
}

// Load restores a snapshot written by Save, auto-scanning tracked plugins for components, Serializable targets, and post-load systems.
func (p *Persistence) Load(basePath, label string, resources ...any) error {
	pm := p.game.pluginManager
	comps := pm.providedComps()
	all := append(pm.saveTargets(), p.game.resources.Persisted()...)
	all = append(all, resources...)
